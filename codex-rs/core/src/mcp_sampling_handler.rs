...
impl SamplingHandler for CodexSamplingHandler {
    async fn create_message(
        ...
        self.notify_sampling_request(&params).await;
        ...
        let system_text = params.system_prompt.clone().unwrap_or_default();
        let items = convert_sampling_messages_to_items(&params)?;
        let mut conversation: Vec<ResponseItem> = if system_text.trim().is_empty() {
            items.into_iter().map(Into::into).collect()
        } else {
            let mut input_items = Vec::<ResponseInputItem>::with_capacity(items.len() + 1);
            input_items.push(ResponseInputItem::Message {
                role: "user".to_string(),
                content: vec![ContentItem::InputText { text: system_text }],
            });
            input_items.extend(items);
            input_items.into_iter().map(Into::into).collect()
        };
        ...
        let sampling_sub_id = ConversationId::new().to_string();
        ...
        loop {
            ...
            } = process_sampling_stream(response_stream, session.clone(), &sampling_sub_id).await?;
        ...
        self.notify_sampling_response(&response_text).await;
...
async fn call_llm_for_sampling(
    ...
    let (account_id, auth_mode) = auth
        .as_ref()
        .map(|a| (a.get_account_id(), Some(a.mode)))
        .unwrap_or((None, None));
...
impl CodexSamplingHandler {
    ...
    async fn notify_sampling_response(&self, response_text: &str) {
